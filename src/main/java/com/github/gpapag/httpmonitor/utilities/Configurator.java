...
public class Configurator {
  ...
  private boolean parseConfigurationFile(String propertiesFile) throws IOException {
    ...
    properties.forEach(
        (propertyKey, propertyValue) -> {
          ...
          System.out.println(
              String.format("Property[%s] Value[%s]", propertyKeyStr, propertyValueStr));
          ...
          switch (propertyKeyStr) {
            ...
            default:
              System.err.println(
                  String.format(
                      "Ignoring unknown Key[%s] with Value[%s]", propertyKeyStr, propertyValueStr));
              break;
          }
        });
  ...
  public long getStatsIntervalSec() {
    return statsIntervalSec == null ? DEFAULT_STATS_INTERVAL_SEC : statsIntervalSec;
  }

  public long getAlertIntervalSec() {
    return alertIntervalSec == null ? DEFAULT_ALERT_INTERVAL_SEC : alertIntervalSec;
  }

  public double getAlertThreshold() {
    return alertThreshold == null ? DEFAULT_ALERT_THRESHOLD : alertThreshold;
  }

  public String getDataFile() {
    return dataFile == null ? DEFAULT_DATA_FILE : dataFile;
  }
}
