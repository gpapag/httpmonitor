...
public class StatsExtractor {
  ...
  public StatsExtractor(@NonNull Configurator configurator) throws IOException {
    ...
    this.alertManager =
        new AlertManager(configurator.getAlertIntervalSec(), configurator.getAlertThreshold());
  }
  ...
  private void printSectionsStats(
      Instant intervalStartTime,
      Instant intervalEndTime,
      List<TrafficStats> topSections,
      long numInvalidRecords) {
    String intervalBounds =
        String.format(
            "\nInterval[%d, %d]",
            intervalStartTime.getEpochSecond(), intervalEndTime.getEpochSecond());
