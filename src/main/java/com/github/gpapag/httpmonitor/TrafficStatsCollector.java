...
public class TrafficStatsCollector {
  ...
  public void update(@NonNull LogRecord logRecord) {
    ...
    if (100 <= statusCode && statusCode <= 199) {
      trafficStatsValue.incrementHitsInfo().incrBytesInfoBy(bytes);
    } else if (200 <= statusCode && statusCode <= 299) {
      trafficStatsValue.incrementHitsSucc().incrBytesSuccBy(bytes);
    } else if (300 <= statusCode && statusCode <= 399) {
      trafficStatsValue.incrementHitsRdir().incrBytesRdirBy(bytes);
    } else if (400 <= statusCode && statusCode <= 499) {
      trafficStatsValue.incrementHitsClnt().incrBytesClntBy(bytes);
    } else if (500 <= statusCode && statusCode <= 599) {
      trafficStatsValue.incrementHitsSrvr().incrBytesSrvrBy(bytes);
    }
  ...
  public List<TrafficStats> retrieveTopK(int k) {
    PriorityQueue<TrafficStats> maxHeap =
        new PriorityQueue<>((o1, o2) -> (-1) * Long.compare(o1.getHits(), o2.getHits()));
